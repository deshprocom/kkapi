module V1
  class MyInvitesController < ApplicationController
    ...
    def index
      ...
      @invite_users = @current_user.user_relation.s_users.active.order(created_at: :desc).page(params[:page]).per(params[:page_size])
    end
    ...
    def indirect
      ...
      @invite_users = @target.user_relation.s_users.active.order(created_at: :desc).page(params[:page]).per(params[:page_size])
    end
