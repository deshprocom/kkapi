  ...
  module Account
    class VcodeServices
      ...
      def call
        ...
        raise_error 'params_missing' if account_id.blank?

        sms_template = send_template(option_type)

        check_permission(option_type, account_id)
      ...
      def send_mobile_vcodes(option_type, sms_template, account_id)
        raise_error 'mobile_format_error' unless UserValidator.mobile_valid?(account_id)
      ...
      def send_email_vcodes(option_type, sms_template, account_id)
        raise_error 'email_format_error' unless UserValidator.email_valid?(account_id)
      ...
      def check_permission(option_type, account_id)
        # 注册和绑定的时候要求用户不存在
        raise_error 'user_already_exist' if option_type.in?(REQUIRE_USER_NOT_EXIST_TYPES) && check_user_exist(account_id)

        # 其它情况都要求用户已存在
        raise_error 'user_not_found' unless option_type.in?(REQUIRE_USER_EXIST_TYPES) || check_user_exist(account_id)
      end
